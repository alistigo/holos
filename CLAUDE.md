...
| Path | Purpose |
|------|---------|
| `ai/memory/MEMORY.md` | Persistent memory across Claude sessions (git-tracked) |
| `ai/skills/` | Skill definitions (source of truth) |
| `ai/commands/` | Slash command definitions (source of truth) |
| `ai/hooks/` | Hook scripts (session-start, etc.) |
| `ai/context/` | Static context documents |
| `.claude/skills` | Symlink → `ai/skills` |
| `.claude/commands` | Symlink → `ai/commands` |
| `.claude/settings.json` | Permissions, hooks config |

`.claude/skills` and `.claude/commands` are **symlinks** to `ai/` — no install scripts needed. Edit files in `ai/` and they take effect immediately.

When creating a new skill, add it to `ai/skills/<skill-name>/SKILL.md`.
When creating a new command, add it to `ai/commands/<command-name>/<command-name>.md`.

## Claude Enhancement Tools

Three tools live as git submodules in `vendor/` and are symlinked into `ai/skills/`:
...
Superpowers injects a structured workflow at session start via `ai/hooks/superpowers-session-start.sh`. It enforces: always write a plan before touching code, use TDD, dispatch parallel agents for independent tasks, and verify before declaring completion. Skills auto-trigger on context — no explicit invocation needed.

### CCPM

CCPM manages the full delivery lifecycle. PRDs go in `ai/prds/`, epics in `ai/epics/`, and GitHub Issues are the source of truth for task tracking.

**Workflow:**
1. "I want to build X" → Claude writes a PRD in `ai/prds/`
2. Approve PRD → Claude creates an epic with numbered tasks in `ai/epics/`

...
operations (e.g. `addElement`) via injecting action descriptors into the HTML of a
new artifact version. The core assumption was that all artifact versions and the published artifact
share the same `window.storage` namespace, so the actions would be saved when a new version is regenerated by AI.
